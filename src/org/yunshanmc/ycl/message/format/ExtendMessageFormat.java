/*
 * 完整Copy自jdk1.8.0_45的java.text.MessageFormat源码
 * 为适应过来而修改的部分：
 *     1.增加从java.text.Format中Copy过来的四个createAttributedCharacterIterator方法(因为它们原来的修饰符是package-private)
 *     2.两个parse方法中对FieldPoison对象的字段直接访问改为getter/setter访问
 *     3.所有MessageFormat改为ExtendMessageFormat
 *     4.删掉public static final format(String,Object...)方法
 * 新增的部分：
 *     1.增加findCustomFormat方法(约1505行)
 *     2.makeFormat方法的switch的default先使用findCustomFormat尝试获取自定义格式化器，获取不到再抛异常
 *     3.增加newThisInstance方法(约1295行)
 *     4.subFormat方法中new MessageFormat改为调用newThisInstance获取
 */
...
/**
 * <code>ExtendMessageFormat</code> provides a means to produce concatenated
 * messages in a language-neutral way. Use this to construct messages
 * displayed for end users.
 *
 * <p>
 * <code>ExtendMessageFormat</code> takes a set of objects, formats them, then
 * inserts the formatted strings into the pattern at the appropriate places.
 *
 * <p>
 * <strong>Note:</strong>
 * <code>ExtendMessageFormat</code> differs from the other <code>Format</code>
 * classes in that you create a <code>ExtendMessageFormat</code> object with one
 * of its constructors (not with a <code>getInstance</code> style factory
 * method). The factory methods aren't necessary because <code>ExtendMessageFormat</code>
 ...
 *
 * <code>ExtendMessageFormat</code> uses patterns of the following form:
 * <blockquote><pre>
 * <i>ExtendMessageFormatPattern:</i>
 *         <i>String</i>
 *         <i>ExtendMessageFormatPattern</i> <i>FormatElement</i> <i>String</i>
 *
 * <i>FormatElement:</i>
 *         { <i>ArgumentIndex</i> }
 ...
 * bundle source files) which strings will be processed by {@code ExtendMessageFormat}.
 ...
 * <p>
 * The first example uses the static method <code>ExtendMessageFormat.format</code>,
 * which internally creates a <code>ExtendMessageFormat</code> for one-time use:
 * <blockquote><pre>
 * int planet = 7;
 * String event = "a disturbance in the Force";
 *
 * String result = ExtendMessageFormat.format(
 ...
 *
 * <p>
 * The following example creates a <code>ExtendMessageFormat</code> instance that
 ...
 *
 * ExtendMessageFormat form = new ExtendMessageFormat(
 ...
 * <blockquote><pre>
 * ExtendMessageFormat form = new ExtendMessageFormat("The disk \"{1}\" contains {0}.");
 ...
 * by a <code>ChoiceFormat</code> in <code>ExtendMessageFormat</code> is treated as special;
 * occurrences of '{' are used to indicate subformats, and cause recursion.
 * If you create both a <code>ExtendMessageFormat</code> and <code>ChoiceFormat</code>
 ...
 * <blockquote><pre>
 * ExtendMessageFormat mf = new ExtendMessageFormat("{0,number,#.##}, {0,number,#.#}");
 ...
 *
 * <p>
 * Likewise, parsing with a {@code ExtendMessageFormat} object using patterns containing
 * multiple occurrences of the same argument would return the last match.  For
 * example,
 * <blockquote><pre>
 * ExtendMessageFormat mf = new ExtendMessageFormat("{0}, {0}, {0}");
...
class ExtendMessageFormat extends Format {
    
    private static final long serialVersionUID = 6479157306784022952L;
    
    /**
     * Constructs a ExtendMessageFormat for the default
     ...
     */
    public ExtendMessageFormat(String pattern) {
        this.locale = Locale.getDefault(Locale.Category.FORMAT);
        applyPattern(pattern);
    }
    
    /**
     * Constructs a ExtendMessageFormat for the specified locale and
     * pattern.
     * The constructor first sets the locale, then parses the pattern and
     * creates a list of subformats for the format elements contained in it.
     * Patterns and their interpretation are specified in the
     ...
     * @since 1.4
     */
    public ExtendMessageFormat(String pattern, Locale locale) {
        this.locale = locale;
        applyPattern(pattern);
    }
    ...
    /**
     * Formats an array of objects and appends the <code>ExtendMessageFormat</code>'s
     ...
     *           (new ExtendMessageFormat(subformat.format(argument), getLocale())).format(argument) :
     ...
     * <code>ExtendMessageFormat</code>'s pattern, producing an
     ...
     * type <code>ExtendMessageFormat.Field</code>, their values are
     * <code>Integer</code> objects indicating the index in the <code>arguments</code>
     * array of the argument from which the text was generated.
     * <p>
     * The attributes/value from the underlying <code>Format</code>
     * instances that <code>ExtendMessageFormat</code> uses will also be
    ...
    public Object[] parse(String source) throws ParseException {
        ...
        if (pos.getIndex() == 0)  // unchanged, returned object is null
            throw new ParseException("ExtendMessageFormat parse error!", pos.getErrorIndex());
     ...
     */
    public Object clone() {
        ExtendMessageFormat other = (ExtendMessageFormat) super.clone();
    ...
    public boolean equals(Object obj) {
        ...
        if (obj == null || getClass() != obj.getClass())
            return false;
        ExtendMessageFormat other = (ExtendMessageFormat) obj;
     ...
     * from <code>ExtendMessageFormat.formatToCharacterIterator</code>.
     *
     * @since 1.4
     */
    public static class Field extends Format.Field {
         ...
         *         resolved.
         * @return resolved ExtendMessageFormat.Field constant
         */
        protected Object readResolve() throws InvalidObjectException {
            if (this.getClass() != ExtendMessageFormat.Field.class) {
            ...
            if (false) { // if (argRecursion == 3){
                ...
                if (obj == null) {
                    ...
                    if (subFormatter instanceof ChoiceFormat) {
                        ...
                        if (arg.indexOf('{') >= 0) {
                            subFormatter = this.newThisInstance(arg, locale);
                            obj = arguments;
                            arg = null;
                        }
                    }
    ...
    protected ExtendMessageFormat newThisInstance(String pattern, Locale locale) {
        return new ExtendMessageFormat(pattern, locale);
    }
    ...
    static final String[] TYPE_KEYWORDS = {
        "",
        "number",
        "date",
        "time",
                            ...
                            StringBuilder[] textSegments)
    {
        ...
        if (segments[SEG_TYPE].length() != 0) {
            ...
            switch (type) {
                ...
                default:
                    newFormat = findCustomFormat(segments[SEG_TYPE], segments[SEG_MODIFIER]);
                    if (newFormat == null) {
                        maxOffset = oldMaxOffset;
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (!isValid) {
            throw new InvalidObjectException("Could not reconstruct ExtendMessageFormat from corrupt stream.");
        }
    }
